...
num_sample = 5; % number of samples taken from each subjects
...
for idx_sub = 1:num_sub
   ...
   % for c = 1:num_stage
   for c = 1:length(select)
       idx_stage = find(labels==select(c));
       idx_pure = [];
...
save([sample_data_path, 'sample3.mat'], 'sample', 'target');