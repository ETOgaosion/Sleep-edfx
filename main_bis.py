# coding=utf-8
# run with Bispectrum and DenseNet
# 23 feature parameters
# writer: Owens
...
def data_loader(args):
    ...
    for mat_idx in os.listdir(args.fea_dir):
        ...
        x1.append(fea_loader['bis_amp'])
        x2.append(fea_loader['bis_pha'])
...
def evaluate_test_set(model, device, x_test, y_test, mean, cov_inv, classes):
    ...
    for seq_idx in range(len(x_test)):
        ...
        y_true += list(seq_target.cpu().numpy())
...
def main():
    ...
    parser.add_argument('--fea_dir', type=str, default='./preparation/fea_bis/', help='feature matrix directory')
